...
export async function POST(req) {
  try {
    ...
    const { error } = await admin.rpc('deliver_order', { p_order_id: orderId, p_admin: `rep:${claim.branch_code}` })
    if (error) return NextResponse.json({ ok:false, error: error.message }, { status:400 })

    await admin.from('orders').update({ delivered_by: deliveredBy || `rep:${claim.branch_code}`, delivered_at: new Date().toISOString() }).eq('order_id', orderId)
    await admin.from('audit_log').insert({ actor:`rep:${claim.branch_code}`, action:'deliver', order_id: orderId, detail: { deliveredBy } })
